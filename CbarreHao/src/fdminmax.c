...
int bsuppvc, bsuppr1,bsupsa,bsupmsa,bsuphh1,bsuphh2,bsupha, bsuppr4, bsuppr2; //booléen pour indiquer la borne sup que l'on souhaite utiliser.
...
int premierebsup = 0;
timestamp_t chronobsup[ZMAX];	/* Dates auxquelles les bsup sont trouvées. */
...
//typejobF3d *job;
{
	...
	tableLB = (int**)malloc(4 * sizeof(int*));
	for(i=0;i<4;i++)
	{
		tableLB[i]=(int*)malloc((nbjobs-1) * sizeof(int));
	}
...
typenoeud *pnd;
{
	...
	if (bs < meilleurCbarre)
	{
		...
		chronobsup[cptbsup] = get_timestamp();
		cptbsup++;
		meilleurCbarre = bs;
...
int* randomSequence1(typenoeud *pnd,int njnp)
{
...
int minSeqM1(int** seqTab,int njnp, int* mTab)
{
	int i, iTemp, mTemp;
    ...
    while(temperature>=tempfinale)
    {
  		...
  		while (j<nbrerepetitions-1)
		{
			...
			dates = calcul_Dates(sequencet);
				...
				lt = algorithm_MCT(dates);
				free_dates(dates);
				dates = calcul_Dates(sequence);
				l = algorithm_MCT(dates);
				free_dates(dates);
...
void decalersequence(int* sequence, int indiceJob, int indiceInser)
{
	int i,temp;

	temp = sequence[indiceJob];

	for(i = indiceJob; i < indiceInser; i++)
	{
	...
	sequence[indiceInser] = temp;

}


int max4(int lb1,int lb2,int lb3,int lb4)
{
	int x=lb1;
	if(x<lb2)
		x=lb2;
...
/*
*	la fonction borneinfetsup calcule la borne inférieur et supérieure d'un noeud.
...
/**
 * int borneinfetsup(pnd, numfils, borninfpere)
 *
...
int numfils;
{
	...
	int LB1=1, LB2=1, LB3=1,LB4=1;
	int flagLB1=1,flagLB2=1,flagLB3=1,flagLB4=1;
	int borninf1,borninf2;
	...
	//Réponse à : premierebinf = borneinfetsup(pilenoeud + 0, 1);
...
//remplir la table dans premier 10000 noeuds.

		borninf=0;
		int couche = pnd->niveau;
		int maxcouche=0;
		if(cptnoeudsevalues<10000)
		{
//LB1
			...
			{				
				...
				free(tabatrier);
				resultpvc = appel_pvc(pnd, job, nbmachines, njnp, seqpvc,0);
			 	if(pnd->niveau == 0)
					LB1 = constanteT + resultSmith + resultpvc ; 
				else		
					LB1 = pnd->cbarre + (njnp * (pnd->d[0])) + constanteT + resultSmith + resultpvc ;
			}
//LB2
			if(flagLB2==1)
			{	
				...
				for(jobi=0;jobi<njnp;jobi++)
				{
					tabatrier[jobi] = job[pnd->reste[jobi]].p[1]; //on met les durées op de tous les jobs sur la premiere machine dans le tableau tabatrier
				}
				tri_a_bulle(tabatrier,njnp);// tabatrier est maintenant trié !
				for(jobi=1;jobi<njnp;jobi++)
				{
				...
				free(tabatrier);
				resultpvc = appel_pvc(pnd, job, nbmachines, njnp, seqpvc,1);
			 	if(pnd->niveau == 0)
					LB2 = constanteT + resultSmith + resultpvc ; 
				else		
					LB2 = pnd->cbarre + (njnp * (pnd->d[1])) + constanteT + resultSmith + resultpvc ;
			}
//LB3
			if(flagLB3==1)
			{	
				...
				if( (tabatrier = (int *)malloc(njnp*sizeof(int))) == NULL){printf("Pb malloc tabatrier\n");}
					for(jobi=0;jobi<njnp;jobi++)
				{
					tabatrier[jobi] = job[pnd->reste[jobi]].p[2]; //on met les durées op de tous les jobs sur la premiere machine dans le tableau tabatrier
				}
				tri_a_bulle(tabatrier,njnp);// tabatrier est maintenant trié !
				for(jobi=1;jobi<njnp;jobi++)
					{
				...
				free(tabatrier);
				resultpvc = appel_pvc(pnd, job, nbmachines, njnp, seqpvc,2);
			 	if(pnd->niveau == 0)
					LB3 = constanteT + resultSmith + resultpvc ; 
				else		
					LB3 = pnd->cbarre + (njnp * (pnd->d[2])) + constanteT + resultSmith + resultpvc ;
			}
//LB4	
			if(flagLB4==1)
			{
				...
				for(jobi=0;jobi<njnp;jobi++)
				{
					tabatrier[jobi] = job[pnd->reste[jobi]].p[3]; //on met les durées op de tous les jobs sur la premiere machine dans le tableau tabatrier
				}
				tri_a_bulle(tabatrier,njnp);// tabatrier est maintenant trié !
				for(jobi=1;jobi<njnp;jobi++)
				{
				...
				if(pnd->niveau == 0)
					...
					LB4=pnd->cbarre + (njnp * (pnd->d[3])) + resultSmith;
			}

			borninf = max4(LB1,LB2,LB3,LB4);
			//	borninf=100;
		...
		else if(cptnoeudsevalues>=10000)
		{
			int maxcouche=max4(tableLB[0][couche],tableLB[1][couche],tableLB[2][couche],tableLB[3][couche]);
			if(tableLB[0][couche]<maxcouche*0.6)
				flagLB1=0;
			if(tableLB[1][couche]<maxcouche*0.6)
				flagLB2=0;
			if(tableLB[2][couche]<maxcouche*0.6)
				flagLB3=0;
			if(tableLB[3][couche]<maxcouche*0.6)
				flagLB4=0;
			constanteT=0;
			for (jobi=0; jobi<njnp; jobi++)
			{
...
/*
	...
	*/	
		...
		if ((bsupsa ==1)&&(njnp>2))
		{
			...
			if(pnd->niveau == 0) 
			{
				seq = (int*)malloc(sizeof(int)*nbjobs);
				M = algorithm_M(seq);
				printf("M=%d\n",M);
				seqResSA = algosa(0.1,0.0001,0.98,50,seq,M);
		...
		bs_pr4=2147483647;
		if ((bsuppr4 ==1)&&(njnp>2)&&(njnp>=nbjobs-1))
		{
...
/*		for(j = 0 ; j < h/2 ; j++)
		...
		fils[i].seqpar[h/2] = numfils;
		for(j = h/2+1 ; j < h+1 ; j++)
		{
			fils[i].seqpar[j] = pilenoeud[pile].seqpar[j-1]; // on rajoute le nouveau job qui est le numfils
		}
*/		
		...
		borneinfetsup(fils+i, i+1, pilenoeud[pile].binf);
		//VERIF printf("cbarre de la seq part du fils : %d\n", fils[i].cbarre);
...
int n;
{
	...
	else
	{
		...
		premierebinf = borneinfetsup(pilenoeud + 0, 1,0);
		...
		while((pile < ZMAX) && (pile > 0))
		{
			...
			if (cptnoeudsevalues%10000==0)
			{
				finish = get_timestamp();
				duration = (finish - start) / 1000000.0L;
				if (duration>1800)
				{
						pile = 0;
				}
		...
		{
			finish = get_timestamp();
			duration = (double)(finish - start) / 1000000.0L;
			...
			printf("\n");
			printf("  Durée avant de trouver l'optimal : %.2f.\n", (double)(chronobsup[cptbsup-1]-start)/1000000.0L);
			...
			fprintf(latex,"%d;",cpt_LB1); // counter of LB1
			fprintf(latex,"%d;",cpt_LB2); // counter of LB2
			fprintf(latex,"%d;",cpt_LB3); // counter of LB3
			fprintf(latex,"%d;",cpt_LB4); // counter of LB3
			//fprintf(latex,"%d;",cpt_LB); // counter of LB1 = LB2 or LB3
		...
		if (output)
				{
					...
					*/ 
					...
					fputs("File;Machine;Job;Opt;1st_LB;LB/Opt%%;1st_UB;LB/UB%%;LB1;LB2;LB3;LB4;Node_Avant;Node_Eval;duration\n", latex);
